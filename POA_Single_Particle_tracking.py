...
class FileSelector:
    @staticmethod
    def select_input1_file():
        root = tk.Tk()
        root.withdraw()
        root.attributes("-topmost", True)
        messagebox.showinfo(
            "Step 1 of 3 - Select input1",
            (
                "input1 - used for track creation"
            ),
            parent=root
        )
        path = filedialog.askopenfilename(
            parent=root,
            title="STEP 1/3 - Select input1.tif (intensity/source stack)",
    ...
    @staticmethod
    def select_input2_file():
        root = tk.Tk()
        root.withdraw()
        root.attributes("-topmost", True)
        messagebox.showinfo(
            "Step 2 of 3 - Select input2",
            (
                "input2 - used for track overlay"
            ),
            parent=root
        )
        path = filedialog.askopenfilename(
            parent=root,
            title="STEP 2/3 - Select input2.tif (blob/segmentation mask)",
            filetypes=[("TIFF files", "*.tif *.tiff"), ("All files", "*.*")]
        )
        root.destroy()
        return path
    ...
    def select_output_folder():
        ...
        root.attributes("-topmost", True)
        messagebox.showinfo(
            "Step 3 of 3 - Select results folder",
            (
                "SELECT THE RESULTS FOLDER NOW\n\n"
                "Choose the folder where all tracking outputs should be written.\n\n"
                "This includes CSV/Excel tables, diagnostic images, plots, and tracking movies.\n\n"
                "The program will write its result files directly into the folder you choose next."
            ),
            parent=root
        )
        folder = filedialog.askdirectory(
            parent=root,
            title="STEP 3/3 - Select folder for tracking results"
        )
        root.destroy()
        return folder

    @staticmethod
    def confirm_selections(input1_path, input2_path, output_folder):
        root = tk.Tk()
        root.withdraw()
        root.attributes("-topmost", True)
        confirmed = messagebox.askyesno(
            ...
            (
                "Please check the three selections before tracking starts:\n\n"
                f"INPUT 1 - intensity/source stack:\n{input1_path}\n\n"
                f"INPUT 2 - blob/segmentation mask:\n{input2_path}\n\n"
                f"RESULTS FOLDER:\n{output_folder}\n\n"
                "Choose YES to start tracking.\n"
                "Choose NO to stop so you can run the script again and select different files."
            ),
            parent=root
        )
        root.destroy()
...
class StackLoader:
    ...
    @staticmethod
    def validate_same_shape(input1_stack, input2_stack):
        if input1_stack.shape != input2_stack.shape:
            raise ValueError(
                f"input1 shape {input1_stack.shape} does not match input2 shape {input2_stack.shape}. "
                "The two stacks must have exactly the same (frames, height, width)."
            )
...
class TrackingPipeline:
    ...
    def run(self, input1_path, input2_path, output_folder):
        progress = ProgressWindow(title="Identity-first particle tracking", maximum=100)

        try:
            TrackColorManager.reset()

            progress.set_progress(1, text="Loading input1 intensity stack and input2 mask stack...")
            input1_stack = StackLoader.load_tiff_stack(input1_path)
            input2_stack = StackLoader.load_tiff_stack(input2_path)
            StackLoader.validate_same_shape(input1_stack, input2_stack)

            # input1 = intensity/source data used for measurements, SUM analysis and overlay background.
            # input2 = segmentation/blob mask used to define which pixels constitute detections.
            tracking_stack = input1_stack
            mask_stack = input2_stack
            overlay_stack = input1_stack

            saver = ResultSaver(output_folder)

            progress.set_progress(8, text="Detecting particles using input2 mask on input1 intensities...")
...
def main():
    ...
    input1_path = FileSelector.select_input1_file()
    if not input1_path:
        print("No input1 stack selected.")
        return

    input2_path = FileSelector.select_input2_file()
    if not input2_path:
        print("No input2 stack selected.")
        return

    output_folder = FileSelector.select_output_folder()
    if not output_folder:
        print("No results folder selected.")
        return

    if not FileSelector.confirm_selections(
        input1_path=input1_path,
        input2_path=input2_path,
        output_folder=output_folder
    ):
        print("Input/output selection was not confirmed.")
        return
